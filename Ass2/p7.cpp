...
int main()
{
	...
	for (int i = 0; i < q; i++)
	{
		...
		while (pt1 < (int)vec.size() || pt2 < (int)tmp.size())
		{
			...
			{
				mn.second--;
				result += mn.first + last;
				tmp.push_back(make_pair(mn.first + last, 1));
			}

			if (mn.second > 1)
			{
				result += 1LL * (mn.second / 2) * (mn.first * 2);
				tmp.push_back(make_pair(mn.first * 2, mn.second / 2));
			}
			mn.second %= 2;
			if (mn.second)
				last = mn.first;
